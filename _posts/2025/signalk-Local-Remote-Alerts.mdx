...
The audible &/or visual behaviour of a client would typically be different for an "alert" vs a "warning" vs an "emergency".
...
In my setup, ALL notifications of the respective type initiate the flows, however the "notification node" can be configured for a single path only if desired. 
In practise, any notification of type "emergency or alarm" result in an ongoing (very loud) beeping until the alarm is cleared (set to "normal" in signalK).
