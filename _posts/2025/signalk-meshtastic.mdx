...
title: Better boat telemetry via Meshtastic
...
Digital switching uses the [Meshtastic's encrypted chat features](https://meshtastic.org/docs/overview/encryption/) to ensure that only registered crew nodes can control the boat.
...
Shared telemetry could even create a network of hyper-local weather stations. You would be able to know what's the actual wind speed behind a corner before you actually sail there. And of course text alerts can be broadcasted. A NAVTEX-to-Meshtastic bridge might be a good place to start.
